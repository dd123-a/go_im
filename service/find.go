...
func FirsFindMsg(database, sendId, id string) (results []ws.Result, err error) {
	//首次查询
	var resultsMe []ws.Trainer
	var resultsYou []ws.Trainer
	sendIdCollection :=conf.MongoDbClient.Database(database).Collection(sendId)
	...
	if len(unReads)>0{
		...
		results,err=FindMany(database,sendId,id,99999999,10)
	}
	...
	_, _ = sendIdCollection.UpdateMany(context.TODO(), filter, bson.M{
		"$set": bson.M{
			"read":    1,
			"ebdTime": time.Now().Unix() + int64(3*month),
		},
	})
	return
}
